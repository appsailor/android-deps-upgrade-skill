...
When asked to upgrade, bump, or update Android/Gradle dependencies, follow the procedure below exactly.

Pass `--check-only` (or if the user says "check only" / "don't apply") to show the upgrade table without making changes.
...
| `androidx.compose:compose-bom` | ⚡ fetch `developer.android.com/develop/ui/compose/bom/bom-mapping` — use the **highest** version shown, including `YYYY.MM.01` patches |
...
If check-only was requested: show the table and stop.
...
2. In `.kt` sources: `import com.google.firebase.ktx.Firebase` → `import com.google.firebase.Firebase`; `import com.google.firebase.FOO.ktx.bar` → `import com.google.firebase.FOO.bar`
